...
-- Джерело: public registry fields exported once from the NextRole job database on 2026-09-14 (companies tagged web3, global boards, crypto Getro collections) plus hand-checked additions; see engine/scripts/jobs-seed.ts
...
INSERT INTO sources (name, label, kind, feed_url, site_url, crypto_only, enabled, terms_note) VALUES ('board:web3career', 'Web3.career', 'jsonld', 'https://web3.career/', 'https://web3.career', 1, 1, 'terms page behind Cloudflare, not verified; robots.txt allows; official free Web3 Jobs API exists (token), switch to it when available') ON CONFLICT(name) DO NOTHING;
