...
## 17. Зміни після рев'ю x402 (12.09)
- `x402_payments.status` має також `unconfirmed` (тайм-аут або відкладений розрахунок); `findStalePayments(db)` дає `verified` старші 5 хв і всі `unconfirmed` для адмінки й cron.
- `request_hash` = sha256(дія + канонічний перевірений вхід + URL ресурсу); ідемпотентний повтор вимагає того самого `request_hash` і НІКОЛИ не виконує дію вдруге.
- Додатковий захист від повтору EVM: UNIQUE `(network, evm_from, evm_nonce)` (EIP-3009).
- `verify()` приймає `input` четвертим аргументом; `PaidRequest.input` обов'язковий.
