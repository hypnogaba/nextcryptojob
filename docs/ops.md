...
## SESSION_SECRET і посилання для подачі
- Особистий ключ профілю-доказу (`/c/<код>?k=`) виводиться з `SESSION_SECRET` (`web/src/lib/card/profile-prefs.ts`).
  Нове значення секрета, крім виходу всіх із сесій, робить усі видані посилання для подачі публічними: вони
  показують лише картку без посилань і контактів. Людям треба скопіювати нове посилання в `/profile`.
