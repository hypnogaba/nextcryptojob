...
const MIGRATIONS = ["0001_core.sql", "0002_auth.sql"];
...
/** Порожня база в пам'яті з міграціями 0001 і 0002. */
export function migratedD1(): TestDb {
  const raw = new DatabaseSync(":memory:");
  // D1 перевіряє зовнішні ключі; тест теж.
  raw.exec("PRAGMA foreign_keys = ON");
  for (const name of MIGRATIONS) {
    raw.exec(readFileSync(new URL(`../../../db/migrations/${name}`, import.meta.url), "utf8"));
  }
  const d1 = {
