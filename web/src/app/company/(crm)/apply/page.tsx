...
import { H3, LINK, PAGE, PageTitle } from "@/components/crm/ui";
    ...
    <section className={`${PAGE} max-w-5xl *:max-w-2xl`}>
      ...
      {application?.status === "pending" ? (
        <div role="status" className="grid gap-2 rounded-xl border border-line bg-surface p-4 sm:p-6">
          <p className={H3}>Application received.</p>
      ...
      {application?.status === "approved" || (company.kind === "agency" && company.status === "active") ? (
        <div role="status" className="grid gap-2 rounded-xl border border-line bg-surface p-4 sm:p-6">
          <p className={H3}>Your agency account is approved.</p>
          <p>
      ...
      {application?.status === "rejected" ? (
        <div role="status" className="grid gap-2 rounded-xl border border-line bg-surface p-4 sm:p-6">
          <p className={H3}>Your agency application was not approved.</p>
      ...
      {application?.status === "needs_info" ? (
        <div role="status" className="grid gap-1 rounded-xl border border-line-strong bg-wash p-4 sm:p-6">
          <p className="font-semibold text-ink">We need more information before we decide:</p>
      ...
      {application && application.status !== "needs_info" ? (
        <dl className="grid gap-3 rounded-xl border border-line bg-surface p-4 text-sm sm:p-6">
          ...
          <div>
            <dt className="font-semibold text-ink-muted">Website and country</dt>
            <dd className="text-ink">
              {application.website}, {countryName(application.country)}
            </dd>
          </div>
          <div>
            <dt className="font-semibold text-ink-muted">Who you recruit for</dt>
            <dd className="whitespace-pre-line text-ink">{application.clientsText}</dd>
          </div>
          {application.volumeText ? (
            <div>
              <dt className="font-semibold text-ink-muted">Hires per quarter</dt>
              <dd className="text-ink">{application.volumeText}</dd>
            </div>
          ) : null}
          <div>
            <dt className="font-semibold text-ink-muted">How you will use candidate profiles</dt>
