...
//   лідерборду так не зібрати: ні ніків, ні адрес, ні контакту, ні слів людини. Без групи
//   "wallets" (власник 16.09, п.4): онлайн-активність гаманця не розклад балу, а сирий доказ,
//   зайвий для чужої людини.
...
export function profileView(input: ProfileInput, mode: ProfileMode): ProfileView {
  ...
  const groups = GROUPS.flatMap(({ key, title, identity }) => {
    // Власник 16.09, п.4: незнайомець на публічній сторінці картки не бачить онлайн-активність
    // людини (кількість транзакцій, трейдів, активні мережі, вік гаманця): це не «розклад балу»
    // (той лишається на звороті картки, джерело/вага/значення/бали), а сирий доказ, зайвий для
