...
export async function finishTelegramLogin(params: URLSearchParams, origin: string, deps: Deps = {}): Promise<string> {
  ...
  // Кнопку натискали з сесією, а повернулись без неї чи з іншою: прив'язувати нема до кого.
  if (flow.linkUserId && session?.id !== flow.linkUserId) return errorPath("session_changed");
