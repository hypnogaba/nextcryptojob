...
/**
 * search_jobs бере вакансії зі сканування тими самими правилами, що й добірка engine
...
describe("Job rules in web are the engine's rules", () => {
  it("roles from the job title: the code is the same as engine/src/digest/roles.ts", () => {
    expect(fromFirstExport(read("./roles.ts"))).toBe(fromFirstExport(read("../../../../engine/src/digest/roles.ts")));
  });

  it("non-crypto companies: the code is the same as engine/src/digest/clean.ts", () => {
    expect(fromFirstExport(read("./clean.ts"))).toBe(fromFirstExport(read("../../../../engine/src/digest/clean.ts")));
  });
  ...
  it("the row sieve (tag, company, title) is the engine's crawlJob up to where it builds the job", () => {
    ...
    const web = read("./pool.ts");
    const body = (src: string, head: string, end: string) => {
      const start = src.indexOf("\n", src.indexOf(head)) + 1;
      return src.slice(start, src.indexOf(end, start));
    };
    const engineSieve = body(engine, "export function crawlJob(r: PoolRow)", "  return {\n    job:");
    expect(engineSieve.trim()).not.toBe("");
    expect(body(web, "export function crawlSieve(r: PoolRow)", "  return { tags, roles };")).toBe(engineSieve);
  ...
  it("the sieve drops the same rows as the engine for the same reason", () => {
    ...
    for (const r of [
      ...
      const e = engineJobs.crawlJob(r);
      const w = crawlSieve(r);
