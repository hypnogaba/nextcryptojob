// Чому вакансія підходить людині: копія engine/src/digest/fit.ts.
//
// Сторінка /jobs пояснює «Jobs for you now» тими самими словами, що й щоденна добірка engine
// (лист, Telegram, sent.why). Worker сайту не збирає код engine, тож код нижче першого export
// дослівно такий самий, як в engine; тест parity.test.ts це звіряє. Правити engine, потім переносити сюди.
import type { RoleKey } from "@/lib/card/roles";
import { type DigestPick, type DigestProfile, foldText, formatSalary, stillOpenNote, workModes } from "./match";
