// Підбір вакансій людині: копія engine/src/digest/match.ts.
//
// Сторінка /jobs показує «Jobs for you now» тими самими правилами, що й щоденна добірка
// engine (роль, місце, одна вакансія на компанію, м'яка зарплата, свіжість, нічого з уже
// надісланого). Worker сайту не збирає код engine, тож код нижче першого export дослівно
// такий самий, як в engine; тест parity.test.ts це звіряє. Правити engine, потім
// переносити сюди (скрипт: вставити все від першого export у engine).
 ...
 * Назва першої мітки з часів, коли вакансії читались з бази попереднього проєкту: вона збережена в sent.source
