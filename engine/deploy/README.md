...
| ~~JobStash (`board:jobstash`)~~ | **закрито 17.09.2026** | вимкнено назавжди | власник дошки написав через `/contact`, що не хоче нашого читання. Того ж дня: `enabled=0`, вакансії видалено з `jobs_cache`, хост заблоковано в коді (`sources/boards.ts`, `BLOCKED_BOARDS`), тож помилковий `UPDATE` джерела не поверне. Компанії, чиї вакансії пішли разом із ним, повернуто з їхніх власних ATS (`scripts/recover-ats.ts`). Публічне правило для власників дощок: `docs/legal/sources.md`, сторінка `/sources` |
...
| Дошки екосистем і фондів на Getro (`job_boards`, 26 з рішенням `discover`) | лише щотижнева розвідка: список компаній дошки і сторінка вакансій лише невідомої реєстру компанії, звідки береться адреса її ATS; вакансій і текстів Getro в базі немає | `JOBS_GETRO_DISCOVERY` (рішення власника); `0` вимикає, у коді типово 0 | джерело з окремими умовами: рішення й оцінка ризику в `RISKS.private.md` (поза репозиторієм). Що саме читаємо: див. «Дошки екосистем і фондів» нижче |
...
Getro ніде немає. Умови джерела й рішення власника: `RISKS.private.md` (поза репозиторієм).
...
| `JOBS_GETRO_DISCOVERY` | рішення власника | розвідка з дошок Getro реєстру `job_boards` («Дошки екосистем і фондів» нижче, умови джерела в `RISKS.private.md`); `0` вимикає, без рядка вимкнено |
...
чверть його давали колекції Getro.
