// Токен роботодавця поруч із вакансією: «$ARB $0.42 · MC $1.9B · +3.1%» і посилання на сайт компанії.
// Дані пише engine (jobs-tokens і ціни після jobs-scan, db/jobs/0004_company_token.sql); тут лише читання
// рядка реєстру, свіжість і формат. Код після першого export дослівно той самий, що web/src/lib/jobs/token.ts
// (web/src/lib/jobs/parity.test.ts): лист, Telegram, сторінка /jobs і search_jobs пишуть однаково.
