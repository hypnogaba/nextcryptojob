...
- `users.roles` не порожній, у `scores` є хоч один рядок людини, `users.digest_paused = 0`
  (колонка з 0011; поки її немає, пауза не діє, у журналі рядок `digest_paused missing`).
...
1. Кеш NextRole, D1 `crypto-jobs-agent` (`0bf4b998-…`), **лише читання**: `jobs-db.ts` віддає назовні
   тільки `select` і відкидає все, крім однієї інструкції SELECT/WITH. Один запит на прогін і лише коли
   комусь пора: вакансії, які NextRole бачив за 3 доби (`fetched_at`), з тегом `web3`, опубліковані
   за 30 днів. Індексу на `fetched_at` там немає навмисно (NextRole зняв його 04.09 заради записів),
   тож це повний прохід: 12.09 `rows_read` = 56 955, D1 63–84 мс, 2 476 рядків відповіді.
...
Чистка кешу NextRole (12.09 на 2 476 живих рядках з тегом web3 лишилось 1 800): список не-крипто
компаній (`clean.ts`, 568 рядків), назви не про нашу аудиторію (`roles.ts NON_CRYPTO_TITLE`: розшифровувачі
аудіо, кухарі, техніки центрів обробки даних, «General Application») і назва мусить мапитись на
нашу роль (`titleRoles`, словник спільний з `web/src/lib/roles/suggest.ts`), разом 108 рядків.
...
(`job_ref` = `nr:<jobs_cache.id>` або `co:<company_jobs.id>`). Далі доставка й статуси `sent`/`failed`.
...
|---|---|---|
| `CF_ACCOUNT_ID`, `CF_API_TOKEN` | так | ті самі, що для нашої бази; токен має читати і `crypto-jobs-agent` |
...
| `JOBS_D1_DATABASE_ID` | ні | інша база вакансій замість `crypto-jobs-agent` |
