...
/** Бюджет фази «стрічка + карта»: дедлайн людини 45 с, домашня сторінка вже з'їла свій таймаут. */
export const SITE_PHASE_MS = 20_000;
...
/** §2: https + хост нижнім регістром (без порту) + шлях без кінцевого "/". Без схеми вважаємо https. */
export function normalizeSite(input: string): string {
  const raw = input.trim();
  // Схема лише з "://": "example.com:443" це хост із портом, а не схема "example.com:".
...
/** Відповідь: ok лише для 2xx; finalUrl = адреса після редиректів. */
type Got = { ok: boolean; text: string; finalUrl: string };

/**
 * GET через safeFetch. Небезпечна адреса летить далі як UnsafeUrlError; решта збоїв дає ok:false.
 * `signal` може бути сигналом фази: його спрацювання дає ok:false, а скасування викликачем
 * (ctx.signal) летить далі.
 */
async function get(url: string, maxBytes: number, ctx: CollectorContext, signal = ctx.signal): Promise<Got> {
  let res: Response;
  try {
    res = await safeFetch(url, { headers: HEADERS, signal }, fetchOpts(ctx, { timeoutMs: TIMEOUT_MS }));
  } catch (e) {
  ...
  if (!res.ok) {
    ...
    return { ok: false, text: "", finalUrl };
  }
  try {
    return { ok: true, text: await readHead(res, maxBytes), finalUrl };
  } catch (e) {
...
const getQuiet = (url: string, maxBytes: number, ctx: CollectorContext, signal?: AbortSignal): Promise<Got> =>
  get(url, maxBytes, ctx, signal ?? ctx.signal).catch((e: unknown) => {
    if (e instanceof UnsafeUrlError) return { ok: false, text: "", finalUrl: url };
    throw e;
  });
...
/** Хост без порту і без "www." на початку. */
const bareHost = (url: string): string => new URL(url).hostname.toLowerCase().replace(/^www\./, "");

async function sitemapUrls(base: string, ctx: CollectorContext, signal: AbortSignal): Promise<number> {
  const root = await getQuiet(`${base}/sitemap.xml`, SITEMAP_BYTES, ctx, signal);
  if (!root.ok) return 0;
  const top = parseSitemap(root.text);
  if (top.children.length === 0) return top.urls;
  // Лише карти того самого сайту і не стиснені: чужі адреси з карти не качаємо. Сайт = хост
  // остаточної відповіді (apex часто переводить на www), "www." на початку не важить.
  const host = bareHost(root.finalUrl);
  const children = top.children.filter((c) => {
    try { const u = new URL(c); return u.protocol === "https:" && bareHost(c) === host && !u.pathname.endsWith(".gz"); } catch { return false; }
  }).slice(0, MAX_CHILD_SITEMAPS);
  const counts = await Promise.all(children.map(async (c) => {
    const r = await getQuiet(c, SITEMAP_BYTES, ctx, signal);
    return r.ok ? parseSitemap(r.text).urls : 0;
  }));
  return counts.reduce((a, b) => a + b, 0);
}

async function feedStats(base: string, homeHtml: string, ctx: CollectorContext, signal: AbortSignal): Promise<{ items: number; dates: number[] }> {
  ...
  const parsed = await Promise.all(candidates.map(async (c) => {
    const r = await getQuiet(c, FEED_BYTES, ctx, signal);
...
export async function collectSite(url: string, ctx: CollectorContext): Promise<Collected<SiteFacts>> {
  return collect("site", ctx, async () => {
    ...
    // Стрічка й карта мають спільний бюджет часу: що не встигло, дає «немає», решта лишається.
    const phase = new AbortController();
    const timer = setTimeout(() => phase.abort(new Error(`site phase over ${SITE_PHASE_MS} ms`)), SITE_PHASE_MS);
    const signal = ctx.signal ? AbortSignal.any([ctx.signal, phase.signal]) : phase.signal;
    let feed: { items: number; dates: number[] }, urls: number;
    try {
      [feed, urls] = await Promise.all([feedStats(base, home.text, ctx, signal), sitemapUrls(base, ctx, signal)]);
    } finally {
      clearTimeout(timer);
      phase.abort();
    }
